...
///
/// @section _7_series_ahrs_example_cpp_license License
///
...
#error This example requires a logging level of at least MICROSTRAIN_LOGGING_LEVEL_INFO_ to work properly
    ...
    while (filterStatus.filter_state < mip::data_filter::FilterMode::AHRS)
    {
...
static void configureEventTriggers(mip::Interface& _device)
{
    ...
    eventTriggerParameters.threshold.first_thres  = 45.0 * M_PI / 180.0;                         // 45 degrees
    eventTriggerParameters.threshold.second_thres = -eventTriggerParameters.threshold.first_thres; // -45 degrees
...
static void initializeFilter(mip::Interface& _device)
{
    ...
    if (!cmdResult.isAck())
    {
        terminate(
            _device,
            cmdResult,
            "Could not enable the %s for magnetometer!\n",
            mip::commands_filter::AidingMeasurementEnable::DOC_NAME
        );
    }
